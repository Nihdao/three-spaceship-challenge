...
const CUSTOM_ASSETS = ["./audio/music/mainMenu.mp3", "./assets/navi.png"];
...
export default function PreloaderScreen() {
  ...
  useEffect(() => {
    ...
    CUSTOM_ASSETS.forEach((path) => {
      fetch(path)
        .then(() => {
          if (alive) setCustomLoaded((n) => n + 1);
        })
        .catch(() => {
          if (alive) setCustomLoaded((n) => n + 1);
        });
  ...
  const r3fDone =
    !active && (progress >= 100 || (total > 0 && loaded >= total));
  const combinedProgress = Math.round(
    (progress + (customLoaded / CUSTOM_ASSETS.length) * 100) / 2,
  );
    ...
    >
      ...
      >
        ...
        >
          ...
          >
            {total > 0
              ? `${loaded + customLoaded} / ${total + CUSTOM_ASSETS.length} ASSETS`
              : "LOADING\u2026"}
          </span>
          <span
            style={{
