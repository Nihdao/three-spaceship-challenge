...
export const CREDITS_SECTIONS = [
  {
    ...
    items: [
      {
        label: "Main Menu",
        entries: [{ title: "Lost Planet (2)", author: "AlkaKrab" }],
      },
      {
        label: "Combat",
        entries: [
  ...
  },
  {
    ...
    items: [
      {
        label: "PolyPizza",
        entries: [{ title: "Quaternius" }, { title: "Mastjie" }],
      },
    ],
  },
  {
    ...
    >
      ...
      <div style={S.modal}>
        ...
        <div style={S.journeyBlock}>
          ...
          >
            Course by{" "}
            <span style={{ color: "var(--rs-text)" }}>Bruno Simon</span>
          </span>
          <a
          ...
          <div style={S.grid}>
            {CREDITS_SECTIONS[0].items.map((group, gi) =>
              group.entries.map((entry, ei) => (
                <Fragment key={`${gi}-${ei}`}>
                  <span style={S.cellLabel}>{ei === 0 ? group.label : ""}</span>
